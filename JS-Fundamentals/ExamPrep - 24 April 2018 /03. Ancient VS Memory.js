function solve(arr) {
    let arrayOfAllNums = arr.join(" ").split(" ");
    for (let i = 0; i < arrayOfAllNums.length; i++) {
        if (arrayOfAllNums[i] === "32656" && arrayOfAllNums[i + 1] === "19759" &&
            arrayOfAllNums[i + 2] === "32763") {
            let size = Number(arrayOfAllNums[i + 4]);
            let copy = arrayOfAllNums.slice(i + 6, i + 6 + size);
